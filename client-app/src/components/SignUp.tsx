...
const SignUp: React.FC = () => {
  ...
  const [isCheckingUserName, setIsCheckingUserName] = useState(false); // Проверка имени
  ...
  const checkUserNameAvailability = async () => {
    try {
      ...
      toast.error('Ошибка проверки имени пользователя.');
      return false;
    } finally {
  ...
  const handleLogInAsGuest = (event: React.FormEvent) => {
    event.preventDefault();
  ...
  <div className={styles.signwindow}>
    <div>
      <h2 className={styles.signtitle}>Регистрация</h2>
      <form className={styles.signform} onSubmit={handleSignUp}>
        <input
        ...
        <button className={styles.signButton} type="submit" disabled={isCheckingUserName}>
          {isCheckingUserName ? 'Проверка...' : 'Зарегистрироваться'}
        </button>
        <button className={styles.signasGuest} type="button" onClick={handleLogInAsGuest}>
          Войти как гость
        </button>
      </form>
